...
describe("analog pin state factories", () => {
  ...
  beforeEach(() => {
    ...
    const event = createTestEvent(digitalReadSetup.id);

    saveSensorSetupBlockData(event).forEach(updater);

    saveSensorSetupBlockData(createTestEvent(analogReadSetup.id)).forEach(
      updater
    );
  });

  test("should be able generate state for analog and digital read setup block", () => {
    const event = createTestEvent(analogReadSetup.id);
